...
int find(char *str) //ASCII
{
    ...
    for(; *str != END; str++)
    {
        ...
        if(n >= 0)
        {
            ...
            if(anodes[n].next[*str / 6] != 0) 
            {
                n = anodes[n].next[*str / 6];    
                if(bnodes[n].next[*str % 6] != 0)
                {
                    n = bnodes[n].next[*str % 6];
                    if(n >= 0) list = anodes[n].list;
                    else list = basic[-n-1].list;     
                }
                else return -1;
            }
...
{
    start();   
    anodes = (A_NODE*)Load_on_RAM(string("trie/light/trie_a"));
    bnodes = (B_NODE*)Load_on_RAM(string("trie/light/trie_b"));
    basic = (BASICNODE*)Load_on_RAM(string("trie/light/trie_basic"));
